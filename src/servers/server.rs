...
impl ServerData {
   ...
   fn send_message(&mut self, message: &[u8]) -> bool {
   ...
   fn handle_rq(&mut self, mut stream: &TcpStream) {
      let mut data = [0 as u8; 50]; // using 50 byte buffer
      while match stream.read(&mut data) {
         Ok(size) => {
            // echo everything!
            println!("Received message");
            self.send_message(&data[0..size]);         
            true
         },
         Err(_) => {
...
pub fn create_server(config_lock: Arc<RwLock<Config>>) {
   ...
   let mut server = ServerData{ addr: SocketAddr::from(([0, 0, 0, 0], config.port)),
                            nodes: nodes };
